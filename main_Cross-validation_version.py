...
y_new = y
optimized_theta = {}
for k in range(5) :  #Ajout de CROSS-VALIDATION
    X = X_new[np.r_[0:20*k, 20*(k+1):], :]
    y = y_new[np.r_[0:20*k, 20*(k+1):]]
    X_test = X_new[20*k:20*(k+1), :]
    y_test = y_new[20*k:20*(k+1)]
    ...
    for i in range(p.size):
        if p[i] == y_test[i]:
            counter += 1
    print('For option ',k,', Train Accuracy: {:.2f}'.format(counter / float(p.size) * 100.0))
    optimized_theta["Option",k] = np.array(theta)
